...
public class AddSha256ToArtifactService implements RestService {
    ...
    public void execute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        if(!authService.canDeployToLocalRepository() || !authService.canDeploy(repoPath)
            ...
            String authErr = String.format(
                    "The user: '%s' is missing the required permissions to trigger a sha256 calculation on path '%s'",
                    authService.currentUsername(), repoPath.toPath());
            log.error(authErr);
    ...
    private void addSha256ToArtifact(RepoPath repoPath) {
        addonsManager.addonByType(ArtifactPropertiesAddon.class).addPropertySha256RecursivelyMultiple(repoPath);
    }
}
