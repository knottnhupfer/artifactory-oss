...
public class IndexXrayService implements RestService<Void> {
    ...
    public void execute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        if (!xrayAddon.isXrayEnabled()) {
            response.error("Xray configuration does not exist on this Artifactory instance or its disabled").responseCode(400);
            log.warn(err, "No Xray configuration found or is disabled.");
            return;
        }
        ...
        if (StringUtils.isBlank(repos)){
            ...
            log.warn(err, "No repos were given to index.");
            return;
        }
