...
public class StorageInfoHelper {
    ...
    public StorageInfoHelper(HttpServletRequest request, RepositoryService repositoryService,
            PropertiesService propertiesService, RepositoryBrowsingService repoBrowsingService,
            ItemInfo itemInfo) {
    ...
    /**
     * is Local/cached/distribution repo.
     *
     * @param repoKey String
     */
    private boolean isLocalRepo(String repoKey) {
        // Search for local/cached/distribution by matching localOrCached omitting remote to cached retrieval
    ...
    private void addStorageInfoProperties(RestBaseStorageInfo storageInfo) {
        ...
        Properties propertiesAnnotatingItem = propertiesService.getProperties(itemInfo.getRepoPath());
        if (!propertiesAnnotatingItem.isEmpty()) {
            for (String propertyName : propertiesAnnotatingItem.keySet()) {
                storageInfo.properties.put(propertyName,
                        Iterables.toArray(Objects.requireNonNull(propertiesAnnotatingItem.get(propertyName)),
                                String.class));
            }
        }
    }
