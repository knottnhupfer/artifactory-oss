...
public class CreateDefaultJcenterRepoService implements RestService {
    ...
    public void execute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        try {
            ...
            String message = "Error while creating repository " + repoKey;
            log.error(message);
            log.debug(message, e);
        }
    }
    ...
    private RemoteRepositoryConfigModel createDefaultModel(String repoKey) {
        ...
        basic.setUrl(RepoConfigDefaultValues.DefaultUrl.urlByType(typeSpecific.getRepoType())); //jcenter
        ...
        if (configDescriptor.getPropertySets().stream()
                .anyMatch(propertySet -> "artifactory".equalsIgnoreCase(propertySet.getName()))) {
