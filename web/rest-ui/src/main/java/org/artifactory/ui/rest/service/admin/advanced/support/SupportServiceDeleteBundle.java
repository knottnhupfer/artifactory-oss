...
public class SupportServiceDeleteBundle<String> implements RestService<String> {
    ...
    public void execute(ArtifactoryRestRequest<String> request, RestResponse response) {
        ...
        if (supportAddon.isSupportAddonEnabled()) {
            ...
            try {
                boolean deleted = supportAddon.deleteBundle(bundle.toString());
                if (!deleted) {
                    response.error(
                            "Support bundle file " + bundle + " was not deleted, check the logs for more details.");
                }
            } catch (UnsupportedOperationException e) {
                response.error("Support bundle operations requires Artifactory Pro.");
            }
        }
    }
}
