...
public class SearchBuildVersionsByVersionsListService implements RestService {
    ...
    public void execute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        BuildNumbersSearchModel buildNumbersModel = (BuildNumbersSearchModel) request.getImodel();
        if(buildNumbersModel.getVersions().size() > ConstantValues.searchUserMaxListSize.getInt()) {
            throw new IllegalArgumentException("Versions list size must be less then " + ConstantValues.searchUserMaxListSize.getInt());
        }
        buildVersionSearchFilter.setNumbers(buildNumbersModel.getVersions());
    ...
    private BuildNumberInfo getBuildGeneralInfo(GeneralBuild build) {
        String buildTime = ISODateTimeFormat.dateTime().print(build.getBuildDate());
