...
public class TestRemoteReplicationService implements RestService<RemoteRepositoryConfigModel> {
    ...
    public void execute(ArtifactoryRestRequest<RemoteRepositoryConfigModel> request, RestResponse response) {
        ...
        try {
            ...
            // Resetting the original payload in case the URL test was a success (because we DO have an error now)
            response.iModel(null);
            response.error(error).responseCode(SC_BAD_REQUEST);
        }
    }

    private void testRemoteReplication(RemoteReplicationDescriptor replicationDescriptor, RemoteRepoDescriptor repo)
            throws IOException {
        if (StringUtils.isBlank(replicationDescriptor.getRepoKey())) {
            replicationDescriptor.setRepoKey(repo.getKey());
        }
        addonsManager.addonByType(ReplicationAddon.class).validateTargetIsDifferentInstance(replicationDescriptor, repo);
    }
