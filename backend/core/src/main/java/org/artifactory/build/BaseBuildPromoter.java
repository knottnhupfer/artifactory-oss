...
public class BaseBuildPromoter {
    ...
    protected Build getBuild(BuildRun buildRun) {
        return buildService.getBuildInternally(buildRun);
    }
     ...
     */
    protected Set<FileInfo> collectItems(Build build, Promotion promotion, BasicStatusHolder multiStatusHolder) {
        Set<FileInfo> itemsToMove = Sets.newHashSet();
        if (noModules(build)) {
            return itemsToMove;
        }
        //Artifacts should be collected
        if (promotion.isArtifacts()) {
            Set<ArtifactoryBuildArtifact> buildArtifactsInfo = getBuildArtifactsAndVerifyInfo(build, promotion, multiStatusHolder);
            for (ArtifactoryBuildArtifact artifact : buildArtifactsInfo) {
                itemsToMove.add(artifact.getFileInfo());
            }
        }
            ...
            for (Map.Entry<Dependency, FileInfo> entry : buildDependenciesInfo.entrySet()) {
                ...
                if (dependency != null) {
                    ...
                    if (org.artifactory.util.CollectionUtils.isNullOrEmpty(
                        ...
                        itemsToMove.add(entry.getValue());
                    }
                }
            }
        }
    ...
    private Set<ArtifactoryBuildArtifact> getBuildArtifactsAndVerifyInfo(Build build, Promotion promotion, BasicStatusHolder multiStatusHolder) {
        ...
        if (!multiStatusHolder.getErrors().isEmpty() && promotion.isFailFast()) {
            throw new ItemNotFoundRuntimeException(errorMessage + ": aborting promotion.");
        }
        return buildArtifactsInfo;
    }
    ...
    protected MoveMultiStatusHolder delete(Set<RepoPath> sourceItemsPaths) {
        try {
            return repositoryService.delete(sourceItemsPaths);
        } catch (Exception e) {
            MoveMultiStatusHolder statusHolder = new MoveMultiStatusHolder();
            statusHolder.error("Error occurred while deleting: " + e.getMessage(), e, log);
            return statusHolder;
        }
    }
    ...
    protected MoveMultiStatusHolder copy(Map<RepoPath, RepoPath> mappingResult, boolean dryRun, boolean failFast) {
        MoveMultiStatusHolder status = new MoveMultiStatusHolder();
        for (Map.Entry<RepoPath, RepoPath> entry : mappingResult.entrySet()) {
            try {
                status.merge(repositoryService.copy(entry.getKey(), entry.getValue(), dryRun, true, failFast));
            } catch (Exception e) {
    ...
    void tagBuildItemsWithProperties(Set<RepoPath> itemsToTag, Properties properties, boolean failFast,
