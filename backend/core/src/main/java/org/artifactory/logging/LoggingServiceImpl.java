...
public class LoggingServiceImpl implements LoggingService {
    ...
    public void importFrom(ImportSettings settings) {
        ...
        if (logFileToImport.exists()) {
            try {
                ...
                Path tempPath = Files
                        .createTempDirectory(Thread.currentThread().getName() + "-" + System.currentTimeMillis());
                File workFile = new File(tempPath.toFile(), logFileToImport.getName());
    ...
    private void convertAndSave(File from, ImportSettings settings) throws IOException {
        ...
        if (source != null) {
            ...
            LoggingVersion.convert(importedVersion,from.getParentFile(), from.getParentFile());
        }
    }
}