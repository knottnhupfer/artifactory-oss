...
public class PasswordExpireNotificationJob extends QuartzCommand {
    ...
    public void onExecute(JobExecutionContext callbackContext) throws JobExecutionException {
        ...
        if(!configService.getMutableDescriptor().getSecurity().getPasswordSettings().getExpirationPolicy().getEnabled()) {
            log.debug("ExpirationPolicy is disabled");
            return;
        }
        if(!configService.getMutableDescriptor().getSecurity().getPasswordSettings().getExpirationPolicy().getNotifyByEmail()) {
