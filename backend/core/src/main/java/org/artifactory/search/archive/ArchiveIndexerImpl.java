 ...
 */
@Service
public class ArchiveIndexerImpl implements InternalArchiveIndexer, ContextCreationListener {
    ...
    public void triggerQueueIndexing() {
        ...
        try {
            boolean indexingSemaphore = conflictGuard.tryToLock(2, TimeUnit.MINUTES);
    ...
    boolean shouldStop() {
        if (conflictGuard == null) {
            log.debug("Locking provider is not yet ready. Skipping job");
            return true;
        }
