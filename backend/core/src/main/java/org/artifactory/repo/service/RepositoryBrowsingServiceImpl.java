...
public class RepositoryBrowsingServiceImpl implements RepositoryBrowsingService {
    ...
    private static class RemoteOnlyBrowsableItemPredicate implements Predicate<RemoteItem> {
        ...
        private RemoteOnlyBrowsableItemPredicate(@Nonnull List<BaseBrowsableItem> localItems) {
            this.names = localItems.stream()
                    .map(BaseBrowsableItem::getName)
                    .collect(Collectors.toSet());
        }

        @Override
        public boolean apply(RemoteItem input) {
            if (input == null) {
                log.error("RemoteOnlyBrowsableItemPredicate: null  RemoteItem not permitted - ignored.");
                return false; // should not encounter null
            }
