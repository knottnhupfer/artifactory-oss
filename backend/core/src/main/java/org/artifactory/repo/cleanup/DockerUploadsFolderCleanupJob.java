...
/**
 * A quartz command job that periodically cleans the "_uploads" folder in docker local repositories
 *
 * @author dudim
 */
@JobCommand(singleton = true,
        ...
        description = "Docker local repositories _uploads cleanup",
        commandsToStop = {
...
public class DockerUploadsFolderCleanupJob extends QuartzCommand {

    @Override
    protected void onExecute(JobExecutionContext callbackContext) throws JobExecutionException {
        ArtifactoryContext artifactoryContext = ContextHelper.get();
        DockerUploadsFolderCleanupService dockerCleaner = artifactoryContext.beanForType(DockerUploadsFolderCleanupService.class);
        dockerCleaner.clean();
    }
}