...
import org.artifactory.resource.BlockedRepoResource;
...
public class DbLocalRepo<T extends LocalRepoDescriptor> extends RealRepoBase<T> implements LocalRepo<T> {
    ...
    public StatusHolder checkDownloadIsNotBlocked(RepoPath repoPath) {
        ...
        if (ContextHelper.get().beanForType(AddonsManager.class).addonByType(XrayAddon.class).isDownloadBlocked(repoPath)) {
            status.error("Artifact download request rejected: " + repoPath.getPath() + " was not downloaded due to "
                            + "the download blocking policy configured in Xray for " + repoPath.getRepoKey() + ".",
