...
createArtDefault() {
    ...
    if [ -e ${artDefaultFile} ]; then
        echo -n "changing ARTIFACTORY_PID..."
        sed --in-place -e "s,export ARTIFACTORY_PID=.*,export ARTIFACTORY_PID=/var/run/artifactory.pid,g;" ${artDefaultFile} || \
                errorArtHome "Could not change ARTIFACTORY_PID in $artDefaultFile"
    else
...
installInitdService() {
    ...
    sed --in-place -e "
     ...
     s%# pidfile: .*%# pidfile: /var/run/artifactory.pid%g;
...
installSystemdService() {
    ...
    s|PIDFile=.*|PIDFile=\/var\/run\/artifactory.pid|g;" ${artSystemdFile}
