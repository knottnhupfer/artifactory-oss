...
public class CentralConfigReader {
    ...
    public MutableCentralConfigDescriptor read(String artifactoryConfigXml, boolean withValidation) {
        return readAndConvert(artifactoryConfigXml, withValidation).getLeft();
    }
     ...
     *
     * @param artifactoryConfigXml The artifactory config xml to be read.
     * @return The up to date artifactory config xml after conversion (if needed), and a flag to indicate whether a conversion has been made.
     */
    public Pair<MutableCentralConfigDescriptor, Boolean> readAndConvert(String artifactoryConfigXml, boolean withValidation) {
        ArtifactoryConfigVersion configVersion = verifyCurrentConfigVersion(artifactoryConfigXml);
        Boolean converted = Boolean.FALSE;
        if ( ! configVersion.isLast()) {
            log.info("Converting artifactory.config.xml version from '{}' to '{}'",
                    configVersion.toString(), ArtifactoryConfigVersion.last());
            artifactoryConfigXml = configVersion.convert(artifactoryConfigXml);
            converted = Boolean.TRUE;
        }
        return new ImmutablePair<>(JaxbHelper.readConfig(artifactoryConfigXml, withValidation), converted);
    }
    ...
    private ArtifactoryConfigVersion verifyCurrentConfigVersion(String configXmlString) {
        if (!configXmlString.contains(Descriptor.NS)) {
            ...
            } else if (guessedConfigVersion == ArtifactoryConfigVersion.last()) {
                throw new RuntimeException(msg +
