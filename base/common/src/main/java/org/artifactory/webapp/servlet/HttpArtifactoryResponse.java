...
public class HttpArtifactoryResponse extends ArtifactoryResponseBase {
    ...
    /**
     * Set the content disposition type to attachment if it is not already set. This will instruct the browser downloading the file to save it
    ...
    public void setContentDispositionAttachment(String filename) {
        if (StringUtils.isBlank(response.getHeader(HttpHeaders.CONTENT_DISPOSITION))) {
            if (ConstantValues.responseDisableContentDispositionFilename.getBoolean() ||
