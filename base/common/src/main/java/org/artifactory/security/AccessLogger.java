...
public abstract class AccessLogger {
    ...
    public enum Action {
        ...
        APPROVE, AUTHENTICATE, FORM_SUBMIT, PROPERTY_ADDED, PROPERTY_REMOVED, PROPERTY_UPDATED, BUILD_CREATE,
        BUILD_PROMOTE, BUILD_DELETE
    }
    ...
    public static void deniedAuthentication(boolean denied, Authentication authentication, String message) {
        logAction(null, Action.AUTHENTICATE, denied, authentication, message);
    }

    public static void buildCreate(String buildName) {
        logAction(null, Action.BUILD_CREATE, false, AuthenticationHelper.getAuthentication(), buildName);
    }

    public static void buildCreateDenied(String buildName) {
        logAction(null, Action.BUILD_CREATE, true, AuthenticationHelper.getAuthentication(), buildName);
    }

    public static void buildPromote(String buildName) {
        logAction(null, Action.BUILD_PROMOTE, false, AuthenticationHelper.getAuthentication(), buildName);
    }

    public static void buildPromoteDenied(String buildName) {
        logAction(null, Action.BUILD_PROMOTE, true, AuthenticationHelper.getAuthentication(), buildName);
    }

    public static void buildDelete(String buildName) {
        logAction(null, Action.BUILD_DELETE, false, AuthenticationHelper.getAuthentication(), buildName);
    }

    public static void buildDeleteDenied(String buildName) {
        logAction(null, Action.BUILD_DELETE, true, AuthenticationHelper.getAuthentication(), buildName);
    }
    ...
    private static void logAction(RepoPath repoPath, Action action, boolean denied, Authentication authentication,
            String username, String message) {

        String actionName = action.name();
        if (authentication != null) {
            String address = RequestThreadLocal.getClientAddress();
            ...
            log.info("[{} {}] {} {} for client : {} / {}.", (denied ? "DENIED" : "ACCEPTED"), actionName,
                    (repoPath != null ? repoPath : ""), message, name, address);
