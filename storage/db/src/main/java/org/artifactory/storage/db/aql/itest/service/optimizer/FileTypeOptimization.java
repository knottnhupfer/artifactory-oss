...
public class FileTypeOptimization extends OptimizationStrategy {
    ...
    public <T extends AqlRowResult> void optimize(AqlQuery<T> aqlQuery, String transformation) {
        ...
        do {
            ...
            change |= removeDuplicateOperators(aqlQuery);
            // Remove last operator
            change |= removeLastOperator(aqlQuery);
            // Remove first operator
            change |= removeFirstOperator(aqlQuery);
            // Remove operator before close parenthesis
            change |= removeOperatorBeforeCloseParenthesis(aqlQuery);
            // Remove operator after open parenthesis
            change |= removeOperatorAfterOpenParenthesis(aqlQuery);
            // Remove parenthesis with operators
            change |= removeEmptyParenthesisWithOperator(aqlQuery);
            // Remove empty parenthesis
            change |= removeEmptyParenthesis(aqlQuery);
            // Remove single operator
            change |= removeSingleOperator(aqlQuery);
        } while (change);
    }
