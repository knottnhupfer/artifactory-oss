...
public class StatsPersistingServiceImpl extends AbstractStatsService implements StatsPersistingService {
    ...
    public void updateDownloadStatsIfNeeded(RepoPath repoPath, Request request, boolean isRealRepo) {
        // No impl as this is an internal implementation but we only access this method through the external one
    }
    ...
    public boolean setStats(RepoPath repoPath, StatsInfo statsInfo) {
        ...
        try {
            ...
            String message = "Cannot set stats for path '" + repoPath + "', file node ID not found.";
            log.debug(message);
            log.trace(message, e);
            return false;
        }
