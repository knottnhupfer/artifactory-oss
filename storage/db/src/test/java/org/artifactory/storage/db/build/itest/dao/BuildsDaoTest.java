...
public class BuildsDaoTest extends BuildsDaoBaseTest {
    ...
    public void testSimpleCreateBuild() throws SQLException {
        BuildEntity build11 = createBuild11();
        assertEquals(createBuild(build11), 1);
        assertTrue(build11.isIdentical(buildsDao.getBuild(11L)));
    }

    public void testCreateBuildWithProps() throws SQLException {
        BuildEntity c2 = createBuild12();
        assertEquals(createBuild(c2), 3);
        assertTrue(c2.isIdentical(buildsDao.getBuild(12L)));
    }

    public void testCreateBuildWithPromotions() throws SQLException {
        BuildEntity c2 = createBuild13();
        assertEquals(createBuild(c2), 3);
        assertTrue(c2.isIdentical(buildsDao.getBuild(13L)));
    }

    public void testCreateBuildWithPropsAndPromotions() throws SQLException {
        BuildEntity c2 = createBuild14();
        assertEquals(createBuild(c2), 5);
        assertTrue(c2.isIdentical(buildsDao.getBuild(14L)));
    }

    public void testCreateBuildWithLongModuleProps() throws SQLException {
        BuildEntity c5 = createBuild15();
        assertEquals(createBuild(c5), 2);
        BuildEntity build = buildsDao.getBuild(15L);
        build.getProperties().forEach(buildProperty ->
                assertTrue(buildProperty.getPropValue().length() <= buildsDao.getDbIndexedValueMaxSize(2048)));
    }

    public void testCreateBuildSameNameAndNumber() throws SQLException {
        ...
        assertEquals(createBuild(otherBa2), 1);
    ...
    public void testDeleteBuilds() throws SQLException {
        doDelete(11L, 1);
        doDelete(12L, 3);
        doDelete(13L, 3);
        doDelete(14L, 5);
    ...
    public void testCreateBuildSameNameNumberDate() throws SQLException {
        ...
        createBuild(badBuild);
    }

    @Test(expectedExceptions = SQLException.class)
    public void testCreateBuildSameId() throws SQLException {
        // TORE: [by fsi] use the error handling to verify we broke the correct constraint
    ...
    private void doDelete(long buildId, int nbDeletedRows) throws SQLException {
        ...
        assertEquals(buildsDao.deleteBuild(buildId, false), nbDeletedRows);
        assertNull(buildsDao.getBuild(buildId));
    }

    private void assertAnyBuild(BuildEntity b) {
        assertNotNull(b);
    ...
    public void fullDelete() throws SQLException {
        ...
        assertEquals(buildsDao.deleteAllBuilds(false), 17);
    }
