...
public class ReadOldBackupsTest {
    ...
    public void testReadingVersion() throws Exception {
        for (ArtifactoryVersion version : ArtifactoryVersion.values()) {
            ...
            String backupVersionFolder = "/backups/" + version.getVersion();
    ...
    public void testSecurityConversion() throws Exception {
        ArtifactoryVersion version = ArtifactoryVersionProvider.v130beta2.get();
        String backupVersionFolder = "/backups/" + ArtifactoryVersionProvider.v130beta2.name();
        File backupFolder = ResourceUtils.getResourceAsFile(backupVersionFolder);
        ArtifactoryVersion backupVersion = BackupUtils.findVersion(backupFolder);
        Assert.assertEquals(backupVersion, version);
        ...
        List<RepoAcl> acls = securityConfig.getRepoAcls();
        Assert.assertEquals(acls.size(), 6, "There should be 6 ACLs");
        Acl<RepoPermissionTarget> anythingAcl = acls.get(acls.indexOf(factory.createRepoAcl(
                factory.createRepoPermissionTarget(PermissionTarget.ANY_PERMISSION_TARGET_NAME,
                        Arrays.asList(PermissionTarget.ANY_REPO)))));
